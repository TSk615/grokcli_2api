...
const PAGE_META = {
  ...
  logs: { title: "任务日志", sub: "查询后台任务结果（SSO 导入、测活、Token 续期等）" },
  models: { title: "模型", sub: "上游模型目录（入库）与探测结果" },
  images: { title: "生图工作台", sub: "使用已登录的 Web 账号池生成图片" },
...
const PAGE_HREF = { overview: "/admin", keys: "/admin/keys", accounts: "/admin/accounts", usage: "/admin/usage", logs: "/admin/logs", images: "/admin/images", models: "/admin/models", settings: "/admin/settings", guide: "/admin/guide" };
let _softNavToken = 0;
...
function pageFromPath(pathname) {
  ...
  if (p.endsWith("/images")) return "images";
...
async function softNavigate(name, opts) {
  ...
  try {
    ...
    try {
      ...
      if (page === "images" && typeof bindImageWorkbench === "function") bindImageWorkbench();
...
function rebindPageControls() {
  ...
  try { bindImageWorkbench(); } catch (_) {}
  ...
  if ($("acc-filter-provider")) {
    accountsProviderFilter = $("acc-filter-provider").value || "";
    $("acc-filter-provider").onchange = () => {
      accountsProviderFilter = $("acc-filter-provider").value || "";
      accountsPage = 1;
      loadAccountsPage({ reset: true });
    };
  ...
  on("btn-import-provider", "onclick", () => importProviderCredentials());
...
async function loadDashboard() {
  ...
  if (page === "overview") {
    ...
    try { bindImageWorkbench(); } catch (e) { console.warn("bindImageWorkbench", e); }
  } else if (page === "guide") {
...
function renderAccountsPage() {
  ...
  if (accountsLoading && !pageItems.length) {
    ...
    <tr data-acc-id="${esc(a.id)}">
      ...
      <td>${esc(a.email || a.label || a.name || "—")}<div class="muted mono" style="font-size:0.72rem">${esc(a.id)}</div><span class="g2a-tag">${esc(String(a.provider || "grok_build").replace("grok_", ""))}</span></td>
...
async function loadAccountsPage({ reset = false } = {}) {
  ...
  if ($("acc-filter-provider")) accountsProviderFilter = $("acc-filter-provider").value || "";
  ...
  const ssoQs = (accountsSsoFilter === "1" || accountsSsoFilter === "0")
    ...
    : "";
  const providerQs = accountsProviderFilter
    ? `&provider=${encodeURIComponent(accountsProviderFilter)}`
    : "";
  try {
    const data = await api(
      `/accounts?page=${encodeURIComponent(page)}&page_size=${encodeURIComponent(pageSize)}` +
      `&q=${encodeURIComponent(q)}&sort=${encodeURIComponent(sort)}${ssoQs}${providerQs}`
    );
...
async function importProviderCredentials() {
  const provider = ($("provider-import-kind") && $("provider-import-kind").value) || "grok_web";
  const webTier = ($("provider-import-tier") && $("provider-import-tier").value) || "basic";
  const payload = ($("provider-import-payload") && $("provider-import-payload").value || "").trim();
  if (!payload) { toast("请粘贴 SSO、Cookie 或 JSON", false); return; }
  const btn = $("btn-import-provider");
  if (btn) { btn.disabled = true; btn.textContent = "导入中…"; }
  try {
    const result = await api("/accounts/import", {
      method: "POST",
      body: JSON.stringify({ provider, payload, web_tier: webTier, merge: true }),
    });
    const out = $("provider-import-result");
    if (out) {
      out.textContent = JSON.stringify(result, null, 2);
      out.hidden = false;
      out.classList.remove("hidden", "is-empty");
    }
    if ($("provider-import-payload")) $("provider-import-payload").value = "";
    toast(`已导入 ${result.count || 0} 个 ${provider} 账号`);
    await loadAccountsPage({ reset: true });
  } catch (e) {
    toast(e.message || "Provider 导入失败", false);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = "加密导入"; }
  }
}
...
let imageWorkbenchBusy = false;

function renderImageResults(items) {
  const host = $("image-results");
  if (!host) return;
  const values = Array.isArray(items) ? items.filter((item) => item && (item.url || item.b64_json)) : [];
  if (!values.length) {
    host.innerHTML = '<div class="g2a-empty">上游没有返回可预览图片</div>';
    return;
  }
  host.innerHTML = values.map((item, index) => {
    const src = item.url || (item.b64_json ? `data:${item.mime_type || "image/jpeg"};base64,${item.b64_json}` : "");
    const href = item.url || src;
    return `<a class="g2a-image-result" href="${esc(href)}" target="_blank" rel="noopener" title="打开原图"><img src="${esc(src)}" alt="生成结果 ${index + 1}" loading="lazy" /></a>`;
  }).join("");
}

async function generateWorkbenchImage() {
  if (imageWorkbenchBusy) return;
  const prompt = ($( "image-prompt")?.value || "").trim();
  if (!prompt) { toast("请先输入提示词", false); $("image-prompt")?.focus(); return; }
  const model = $("image-model")?.value || "Web/grok-imagine-image-2.0";
  const aspectRatio = $("image-ratio")?.value || "16:9";
  const count = Math.max(1, Math.min(4, Number($("image-count")?.value || 1) || 1));
  const nsfw = !!$("image-nsfw")?.checked;
  const button = $("btn-image-generate");
  const status = $("image-status");
  imageWorkbenchBusy = true;
  if (button) { button.disabled = true; button.textContent = "生成中…"; }
  if (status) status.textContent = `${model} · ${count} 张 · Resin 出站`;
  try {
    const result = await api("/images/generations", {
      method: "POST",
      body: JSON.stringify({ model, prompt, n: count, aspect_ratio: aspectRatio, response_format: "url", enable_nsfw: nsfw }),
    });
    renderImageResults(result && result.data);
    if (status) status.textContent = `完成 ${(result && result.data && result.data.length) || 0} 张`;
    toast("图片生成完成");
  } catch (e) {
    if (status) status.textContent = e.message || "生成失败";
    toast(e.message || "图片生成失败", false);
  } finally {
    imageWorkbenchBusy = false;
...
function bindImageWorkbench() {
  const button = $("btn-image-generate");
  if (!button) return;
  on("btn-image-generate", "onclick", generateWorkbenchImage);
}
