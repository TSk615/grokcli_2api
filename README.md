...
| **逐次故障切换** | 首账号先租约再加载；只有上一尝试失败后才加载下一个，避免预构造 2 万账号 payload 或绕过 inflight |
| **用量降载** | 日汇总全量保留；普通成功明细确定性采样 5%，失败、超时、5xx、慢请求和审计窗口强制保留；明细按月分区 |
| **持久化队列** | Compose 增加独立 writer，Redis 启用 AOF `everysec`、持久卷与 `noeviction` |
...
```bash
git clone https://github.com/TSk615/grokcli_2api.git
cd grokcli_2api
cp .env.example .env
...
> - 容器：`grokcli-2api-lite`、`grokcli-2api-lite-writer`、`grokcli-2api-lite-redis`、`grokcli-2api-lite-postgres`
...
- 备份重点：**PostgreSQL 卷**（`grok2api_pg`）和 Redis AOF 卷（`grok2api_redis`）；AOF `everysec` 仍接受极端约 1 秒窗口
- gateway 默认只写 Redis 热计数与 Streams；独立 writer 每 1 秒或 500 条批量落 PostgreSQL
- `/metrics` 暴露 writer Stream lag、pending、DLQ、批量大小和提交耗时
...
```bash
# grok2api/app.py 中 APP_VERSION 与 README 当前版本保持一致
git add <本次版本文件>
git commit -m "release: v1.9.93 async writer and indexed account pool"
git push origin main
```

本仓库不自动发布容器镜像；如后续需要，可手动触发 `docker-publish.yml`。
...
.github/workflows/docker-publish.yml     # 可选手动触发的 GHCR 多架构构建
```
...
- **v1.9.94**（当前）
  ...
  - 后续媒体扩展：Web 图片编辑与默认 480P/6 秒文生视频已接入；视频接口说明见 [Web 视频](docs/web-video.md)
- **v1.9.93**
  ...
  - **逐次 failover**：仅租约和加载当前尝试账号，失败后再取下一个，最多保持现有 4 次尝试
  - **用量降载**：全量日汇总、默认 5% 成功明细采样、失败/慢请求/审计强制保留；新增缓存与 reasoning token 汇总
  - **月分区与保留策略**：新明细写入 `usage_events_partitioned`，兼容查询历史表，默认保留 30 天
  - **部署与可观测**：Compose 增加 writer 服务和 Redis AOF；`/metrics` 增加队列 lag、pending、DLQ、批量与 PG 提交指标
  - 回归：`scripts/_test_async_writer_architecture.py`（9 项）及冷却、轮询分散、CPA 粘性、免费额度踢出既有测试
- **v1.9.92**
