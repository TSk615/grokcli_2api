...
_SCHEMA_MIGRATIONS = (
    ...
    "ALTER TABLE account_pool ADD COLUMN IF NOT EXISTS state_version BIGINT NOT NULL DEFAULT 0",
    ...
    "ALTER TABLE usage_daily ADD COLUMN IF NOT EXISTS cache_read_tokens BIGINT NOT NULL DEFAULT 0",
    "ALTER TABLE usage_daily ADD COLUMN IF NOT EXISTS cache_creation_tokens BIGINT NOT NULL DEFAULT 0",
    "ALTER TABLE usage_daily ADD COLUMN IF NOT EXISTS reasoning_tokens BIGINT NOT NULL DEFAULT 0",
    "ALTER TABLE usage_daily ADD COLUMN IF NOT EXISTS cache_hit_requests BIGINT NOT NULL DEFAULT 0",
    ...
    # queryable so upgrades do not require a blocking historical rewrite.
    """
    CREATE TABLE IF NOT EXISTS usage_events_partitioned (
    ...
    """
    CREATE OR REPLACE VIEW usage_events_all AS
    ...
    # message is redelivered after a writer crash between COMMIT and XACK.
    """
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_writer_inbox_processed_at ON writer_inbox (processed_at)",
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            for stmt in _SCHEMA_MIGRATIONS:
                cur.execute("SAVEPOINT g2a_schema_migration")
                try:
                    cur.execute(stmt)
                    cur.execute("RELEASE SAVEPOINT g2a_schema_migration")
                except Exception:
                    # Keep later idempotent migrations runnable after one
                    # optional/legacy statement fails.
                    cur.execute("ROLLBACK TO SAVEPOINT g2a_schema_migration")
                    cur.execute("RELEASE SAVEPOINT g2a_schema_migration")
        conn.commit()
    _schema_ready = True
