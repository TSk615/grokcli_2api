...
async def admin_status(request: Request):
    ...
    if _status_reg_cache is not None and now - _status_reg_at < 30:
        ...
        reg_status = {
            "available": False,
            "error": "protocol registration removed",
            "engine": None,
        }
...
def _load_saved_sso_records() -> list[dict[str, Any]]:
    ...
    # Protocol registration sessions are no longer available; durable account /
    # import_sso backups remain the source of truth for SSO reconcile.
    return records
